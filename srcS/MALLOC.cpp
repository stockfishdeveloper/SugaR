...
void CREATE_MEM2(void** A,uint64_t size)
{
	...
	if(num!=-1)
	{
		(*A)=shmat(num,NULL,0x0);
		large_use=TRUE;
		printf("info string %llu Mb Call down large memory pages if they are provided by the operating system.. Ok LP found!\n",size>>20);
	}
}
...
void CREATE_MEM2(void** A,uint64_t size)
{
	...
	{
		large_use=TRUE;
		printf("info string Call down large memory pages if they are provided by the operating system.. Ok LP found! %llu Mb\n",size>>20);
	}
}
